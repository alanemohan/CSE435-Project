...
// Mock data for demonstration when no alerts exist
const MOCK_ALERTS: CommunityAlert[] = [
  {
    id: '1',
    title: 'SBI KYC Verification Scam',
    description: 'Fraudsters posing as SBI officials asking customers to complete KYC by clicking suspicious links. They request OTP and card details.',
    alert_type: 'phishing',
    severity: 'high',
    region: 'Maharashtra',
    reported_count: 156,
    first_reported_at: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  },
  {
    ...
    description: 'Fake Amazon/Flipkart data entry job offers requiring upfront payment of ₹2,999 to ₹5,999 for registration or training kit.',
    alert_type: 'job_scam',
    severity: 'high',
    region: 'Pan India',
    reported_count: 89,
    first_reported_at: new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString(),
    last_reported_at: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
    is_active: true,
  },
  {
    id: '3',
    title: 'Customs/Courier Parcel Scam',
    description: 'Calls claiming your parcel is stuck at customs with illegal items. They demand payment to "release" the parcel and avoid arrest.',
    alert_type: 'impersonation',
    severity: 'medium',
    region: 'Delhi NCR',
    reported_count: 67,
    first_reported_at: new Date(Date.now() - 21 * 24 * 60 * 60 * 1000).toISOString(),
    last_reported_at: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
    is_active: true,
  },
  {
    id: '4',
    title: 'Instant Loan App Harassment',
    description: 'Predatory loan apps like "CashBean" and "MoneyTap" clones accessing contacts and photos, then blackmailing users for repayment.',
    alert_type: 'loan_scam',
    severity: 'high',
    region: 'Karnataka',
    reported_count: 234,
    first_reported_at: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString(),
    last_reported_at: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString(),
    is_active: true,
  },
  {
    id: '5',
    title: 'Electricity Bill Disconnection',
    description: 'SMS claiming your electricity will be disconnected today unless you pay immediately via provided link. Uses fear tactics.',
    alert_type: 'phishing',
    severity: 'medium',
    region: 'Uttar Pradesh',
    reported_count: 45,
    first_reported_at: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
    last_reported_at: new Date(Date.now() - 12 * 60 * 60 * 1000).toISOString(),
    is_active: true,
  },
];
...
export default function CommunityAlerts() {
  ...
  // Use database alerts primarily, fallback to mock only if DB is empty
  const alerts = dbAlerts && dbAlerts.length > 0 ? dbAlerts : MOCK_ALERTS;
  const isUsingMockData = !dbAlerts || dbAlerts.length === 0;
      ...
      <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
         ...
         <div className="relative overflow-hidden rounded-2xl border border-warning/20 p-8" style={{ background: 'var(--gradient-hero)' }}>
           ...
           <div className="relative z-10">
             ...
             <p className="text-muted-foreground max-w-xl">
               Real-time feed of trending scam patterns reported by our community. Stay informed and protected.
             </p>
           </div>
         </div>
         ...
         {filteredAlerts.length === 0 && (
           <div className="glass-card rounded-2xl p-16 text-center">
             ...
             <p className="text-muted-foreground max-w-md mx-auto">
               No alerts match your current filters. Try adjusting your search or check back later for new reports.
             </p>
           </div>
         )}
         ...
         <div className="glass-card rounded-2xl p-6 border-primary/20">
           <div className="flex items-start gap-4">
             ...
             <div>
               <p className="font-display font-semibold mb-1">Community-Powered Alerts</p>
               <p className="text-sm text-muted-foreground leading-relaxed">
                 These alerts are generated from anonymized user reports. All personal information is removed to protect privacy. If you encounter a scam, analyzing it through our Scam Analyzer helps protect the community.
               </p>
             </div>
           </div>
         </div>
